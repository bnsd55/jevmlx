...
``<out>/RUNBOOK.md``:
 ...
 9. ``README.md`` leaderboard regen + freshness check. The M5 agent commits
    ``benchmarks/results/<folder>`` AND ``README.md`` in the same commit
    (the results-check CI enforces a fresh leaderboard block).
...
def plan_steps(
    ...
    *,
    ...
    # is fresh (check_results --check-readme fails if stale). Always appended
    # after summary. The M5 agent commits README.md + benchmarks/results/
...
def _execute_readme(step: Step, log) -> int:
    """The readme step: regenerate the README leaderboard block and verify it
    is fresh.
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Footer: remind the operator to include README.md in the results commit.
    if not failures:
        with (out / "RUNBOOK.md").open("a", encoding="utf-8") as f:
            f.write(
                "\n---\n"
                "**Results commit**: include `README.md` (regenerated by the readme "
                "step) in the same commit as `benchmarks/results/<folder>`. "
                "The results-check CI enforces a fresh leaderboard block.\n"
            )
