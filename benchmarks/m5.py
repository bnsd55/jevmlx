...
def plan_steps(
    ...
    *,
    ...
    if results_pr:
        # The results-PR step: regenerate the README leaderboard, verify it is
        # fresh (check_results --check-readme fails if stale), then commit
        # README.md + benchmarks/results/ and open a PR. Runs in the repo root.
        steps.append(
            Step(
                id="results-pr",
...
def _execute_results_pr(step: Step, log) -> int:
    """The results-PR step: regenerate the README leaderboard, verify it is
    fresh, then commit README.md + benchmarks/results/ and open a PR.

    Runs in the repo root (step.cwd = REPO_ROOT). Uses the same venv's
    python. Fails (exit 1) if check_results --check-readme finds the README
    stale after regen (a real contract violation, not a skip).
    """
    py = sys.executable
    ...
    # 2. Verify the README is fresh (fail the step if stale).
    rc = _run((py, "-m", "benchmarks.check_results", "--check-readme", str(readme)))
    if rc != 0:
        return rc

    # 3. Commit README.md + benchmarks/results/ and open a PR.
    #    On CI / headless, gh may not be available — the commit + push still
    #    run; the PR creation is best-effort.
    rc = _run(("git", "add", str(readme), str(results_dir)))
    if rc != 0:
        return rc
    rc = _run(("git", "commit", "-m", "results: regenerate README leaderboard"))
    if rc != 0:
        # Nothing to commit (no changes) — still success.
        return 0
    # Push to a results branch and open a PR (best-effort).
    branch = f"results-{_now().split()[0].replace('-', '')}"
    ...
    if gh:
        _run(
            (
                ...
                "create",
                "--title",
                f"Results: regenerate README leaderboard ({branch})",
                "--body",
                "Auto-generated by the m5 results-PR step.",
            )
        )
    return 0
...
def execute_step(step: Step) -> int:
    ...
    with log_path.open("w", encoding="utf-8") as log:
        ...
        if step.in_process:
            if step.id == "results-pr":
                return _execute_results_pr(step, log)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--results-pr",
        action="store_true",
        help="append the results-PR step: regenerate the README leaderboard, "
        "verify freshness, commit README.md + benchmarks/results/ and open a PR",
    )
