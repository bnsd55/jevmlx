...
global optimum is the union of per-component optima. There are no
component-size or implies-fan-in caps: a 64-option field (the schema max)
enumerates at most 2^64 in the pathological single-component case, but real
components are small, and every complete candidate is scored by the same
...
__all__ = ["select_constrained_set", "is_feasible", "components_of"]
...
def select_constrained_set(
    ...
    if not comps:
        # No constraints touch any option (compile should have rejected an
        # empty list; a constraints list of only unknown types lands here).
        best_sel = set(proposal) | forced_by(set(proposal))
    else:
        for comp in comps:
        ...
        touched: set[str] = comp_opts
        free_selected = {o for o in proposal if o not in touched}
