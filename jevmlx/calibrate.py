...
class CalibrationBundle:
    ...
    ``load`` accepts both the review shape and the legacy
    ``{"temperature": .., "multi": {"a": .., "b": ..}}`` the CLI writes.
    """
    ...
    def from_payload(cls, payload: dict) -> "CalibrationBundle":
        ...
        if multi is not None:
            ...
            try:
                ...
                raise ValueError('calibration["multi"] must carry numeric "a" and "b"') from exc
        return cls(
