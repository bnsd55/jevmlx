...
def test_p5_option_plan_cache_reuses_across_rotations():
    ...
    plan1 = schema_canonical.compile_slot_plan(
        tok, make_test_renderer(tok, schema_canonical, "slots")
    )
    ...
    plan2 = schema_rotated.compile_slot_plan(tok, make_test_renderer(tok, schema_rotated, "slots"))
...
def test_p5_option_plan_cache_no_cross_contamination_different_choices():
    ...
    plan_alpha = schema_alpha.compile_slot_plan(tok, make_test_renderer(tok, schema_alpha, "slots"))
    ...
    plan_xyz = schema_xyz.compile_slot_plan(tok, make_test_renderer(tok, schema_xyz, "slots"))
    ...
    schema_alpha_c.compile_slot_plan(
        tok_cached, make_test_renderer(tok_cached, schema_alpha_c, "slots")
    )  # populate cache
    schema_xyz_c = StructuredSchema(
        {"action": {"type": "enum", "description": "d", "choices": ["W", "X", "Y", "Z"]}}
    )
    schema_xyz_c.compile_slot_plan(
        tok_cached, make_test_renderer(tok_cached, schema_xyz_c, "slots")
    )  # populate cache for XYZ
